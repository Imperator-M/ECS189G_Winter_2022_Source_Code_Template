...
class Dataset_Loader(dataset):
    ...
    def load(self):
        ...
        print('Loading {} dataset...'.format(self.dataset_source_file_name))
        ...
        if self.dataset_source_file_name == 'cora':
            ...
            for i in range(num_classes):
                # Randomly select 20 indices for training
                train_idx = np.random.choice(label_indices[i], size=num_train_per_class, replace=False)
                # Randomly select 150 indices for testing
                test_idx = np.random.choice(np.setdiff1d(label_indices[i], train_idx), size=num_test_per_class,
                                            replace=False)
                # Add the indices to the training and testing index lists
            ...
            for i in range(num_classes):
                # Randomly select 20 indices for training
                train_idx = np.random.choice(label_indices[i], size=num_train_per_class, replace=False)
                # Randomly select 200 indices for testing
                test_idx = np.random.choice(np.setdiff1d(label_indices[i], train_idx), size=num_test_per_class,
                                            replace=False)
                # Add the indices to the training and testing index lists
                idx_train.extend(train_idx)
        ...
        elif self.dataset_source_file_name == 'cora-small':
            idx_train = range(5)
            idx_val = range(5, 10)
            idx_test = range(5, 10)
