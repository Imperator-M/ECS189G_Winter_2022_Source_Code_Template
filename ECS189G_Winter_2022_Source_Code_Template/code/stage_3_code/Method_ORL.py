...
class Method_ORL(method, nn.Module):
    ...
    def __init__(self, mName, mDescription):
        ...
        self.fc1 = nn.Linear(32 * 25 * 20, 40)
    ...
    def train(self, train_data, test_data, model):
        ...
        for epoch in range(self.max_epoch):
            ...
            for i, (images, labels) in enumerate(train_loader):
                ...
                if i % 360 == 0:  # 50*32 = 1600 per
                    epochNum.append(epoch)
                    loss.append(train_loss.item())
                    print('Epoch : {} \tLoss: {:.6f}\t Accuracy:{:.3f}%'.format(
                        epoch, train_loss.item(), float(correct * 100) / float(self.batch_size * (i + 1))))
    ...
    def run(self):
        ...
        model = Method_ORL('convolutional neural network', '') #I was having problems getting the model from the script so I just added it manually here
